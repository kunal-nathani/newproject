		...
		<h3 class="text-center" style="background-color:#272327;color: #fff;">My Details</h3>
			<div class="formstyle" style="float: right;padding:20px;border: 1px solid lightgrey;margin-right:415px; margin-bottom:30px;background-color:#f3f3f8;color:#141313;">
				<form action="" method="post" class="text-center form-group">
					<label>
						Your Name: <input type="text" name="name" value="<?php echo $fname; ?>" placeholder="Full name" required>
					</label><br><br>
					...
					<label>
						Mobile: <input type="text" name="contact" value="<?php echo $phone1; ?>" placeholder="contact no" required="required" pattern="[0-9]{10,11}" title="please enter only numbers between 10 to 11 for mobile no."/>
					</label><br><br>
 					
 					<label>
						Address: <input type="text" name="address" value="<?php echo $addr; ?>" placeholder="address" required>
					</label><br><br>
					<label>
						Blood Group: <input type="email" name="email" value="<?php echo $blood; ?>" placeholder="email" disabled>
					</label><br><br>
...
<br><br><br>
			...
			<!-- update information -->
				...
				<?php
							...
							$sql="UPDATE patient SET name='" .$_POST["fname"]. "' ,age='" .$_POST["age"]."' , contact='" .$_POST["phone1"]. "',address='" .$_POST["addr"]. "', email='".$_POST["email"]."' WHERE email='" .$_SESSION["email"]. "'";
